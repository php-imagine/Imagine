...
final class Drawer implements DrawerInterface
{
    ...
    {
        imagesetthickness($this->gd->resource, max(1, (int) $thickness));
        if (false === imagearc(
            $this->gd->resource, $center->getX(), $center->getY(),
    ...
    public function line(PointInterface $start, PointInterface $end, Color $color, $thickness = 1)
    {
        imagesetthickness($this->gd->resource, max(1, (int) $thickness));
        if (false === imageline(
            $this->gd->resource, $start->getX(), $start->getY(),
    ...
    public function dot(PointInterface $position, Color $color)
    {
        if (false === imagesetpixel(
            $this->gd->resource, $position->getX(), $position->getY(),
            $this->getColor($color)
        )) {
    ...
    public function polygon(array $coordinates, Color $color, $fill = false, $thickness = 1)
    {
        ...
        if (false === $callback(
            $this->gd->resource, $points, count($coordinates),
            $this->getColor($color)
        )) {
    ...
    public function text($string, AbstractFont $font, PointInterface $position, $angle = 0)
    {
        ...
        if (false === imagealphablending($this->gd->resource, true)) {
            throw new RuntimeException('Font mask operation failed');
        }

        if (false === imagefttext(
                $this->gd->resource, $fontsize, $angle, $x, $y,
                $this->getColor($font->getColor()), $fontfile, $string
            )) {
            throw new RuntimeException('Font mask operation failed');
        }

        if (false === imagealphablending($this->gd->resource, false)) {
            throw new RuntimeException('Font mask operation failed');
        }
